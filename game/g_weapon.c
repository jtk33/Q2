...
Fires a single blaster bolt.  Used by the blaster and hyper blaster. Now a portal gun
=================
*/
void portal1_touch(edict_t* ent, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	...
	if (portal2) {
		...
		tppos[0] += forward[0] * 100;
		tppos[1] += forward[1] * 100;
		tppos[2] += forward[2] * 100;
...
qboolean portl = false;
qboolean p1 = false;
qboolean p2 = false;
void blaster_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf, vec3_t start, vec3_t aimdir)
{
	int		mod;
	if (portl)
	{
	...
	else //Let's try doing the portal thing here
	{
		gi.WriteByte(svc_temp_entity);
		...
		if (portl)
		{
			...
			portal1 = G_Spawn();
			VectorCopy(self->s.origin, portal1->s.origin);
			VectorCopy(angles, portal1->movedir);
			VectorCopy(angles, portal1->s.angles);
			portal1->solid = SOLID_TRIGGER;
			portal1->s.effects |= EF_BLUEHYPERBLASTER;
			portal1->s.effects |= EF_ROTATE;
			VectorSet(portal1->mins, -16, -16, -24);
			VectorSet(portal1->maxs, 16, 16, 32);
			portal1->s.modelindex = gi.modelindex("models/weapons/black/tris.md2");
		...
		if (!portl)
		{
			...
			portal2 = G_Spawn();
			VectorCopy(self->s.origin, portal2->s.origin);
			VectorCopy(angles, portal2->movedir);
			VectorCopy(angles, portal2->s.angles);
			portal2->solid = SOLID_TRIGGER;
			portal2->s.effects |= EF_HYPERBLASTER;
			portal2->s.effects |= EF_ROTATE;
			VectorSet(portal2->mins, -16, -16, -24);
			VectorSet(portal2->maxs, 16, 16, 32);
			portal2->s.modelindex = gi.modelindex("models/weapons/black/tris.md2");
...
void bfg_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	vectoangles(plane->normal, angles);
	VectorCopy(angles, self->movedir);
...
void fire_bfg(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius)
{
	...
	bfg->s.effects |= EF_BFG | EF_HYPERBLASTER;
	VectorClear(bfg->mins);
	VectorClear(bfg->maxs);
	bfg->s.modelindex = gi.modelindex("models/weapons/black/tris.md2");
	bfg->owner = self;
	bfg->touch = bfg_touch;
